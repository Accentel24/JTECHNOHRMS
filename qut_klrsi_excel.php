...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='klbilling') or ($tsname=='sumanthpotluri')){
      $y="select * from klqot_bill where status='RUn Paid' ";   
    }else{
      $y="select * from klqot_bill where status='RUn Paid' and user='$tsname' ";
	 }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KERALA Raised Invoice LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, mb_strtoupper('KL','UTF-8'));
...
header('Content-Disposition: attachment;filename="klraisedinvoicelist.xlsx"'); //tell browser what's the file name
