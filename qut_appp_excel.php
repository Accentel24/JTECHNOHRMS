...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='apbilling')){
		 $y="select * from qot_bill where status='Un Paid' ";
		 }else{
		 $y="select * from qot_bill where status='Un Paid' and user='$tsname' ";
		}
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'AP PAYPEMENT PENDING LIST');
...
header('Content-Disposition: attachment;filename="appaymentpending.xlsx"'); //tell browser what's the file name
