...
$y="SELECT * FROM add_tnqot1    ORDER BY id desc";   
...
while($row10	=	$result->fetch_assoc()){
    $id1=$row10['id1'];
    $id2=$row10['id'];
    $y10="SELECT * FROM add_tnqot where id='$id1'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, mb_strtoupper(date('d-m-Y',strtotime($row['ro_date'])),'UTF-8'));
		...
		$ds1="select * from add_tnqot1 where id1='$id1' and id='$id2'";
...
header('Content-Disposition: attachment;filename="tnquotationsdetailslist.xlsx"'); //tell browser what's the file name
