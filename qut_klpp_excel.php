...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='klbilling') or ($tsname=='sumanthpotluri')){
      $y="select * from klqot_bill where status='Un Paid' ";   
    }else{
      $y="select * from klqot_bill where status='Un Paid' and user='$tsname' ";
	 }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KERALA PAYPEMENT PENDING LIST');
...
header('Content-Disposition: attachment;filename="klpaymentpending.xlsx"'); //tell browser what's the file name
