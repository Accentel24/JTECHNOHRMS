...
include 'PHPExcel-1.8/Classes/PHPExcel/Writer/Excel2007.php';
 $tsname=$_GET['user'];
//$datatable="add_klqot";
 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='apbilling') or ($tsname=='Srujith.Nimmagadda')){
 $y="SELECT * FROM qot_bill where status='payment pending'   ";
	  }else{ 
 $y="SELECT * FROM qot_bill where status='payment pending' and user='$tsadmin'  "; 
	 }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:Q1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'AP TO BE RAISE  QUOTATION LIST');
...
$objPHPExcel->getActiveSheet()->SetCellValue('L6', 'Bill Received Date');
$objPHPExcel->getActiveSheet()->SetCellValue('M6', 'Ro Amount');
$objPHPExcel->getActiveSheet()->SetCellValue('N6', 'Tot Service');
$objPHPExcel->getActiveSheet()->SetCellValue('O6', 'Tot Gst');
$objPHPExcel->getActiveSheet()->SetCellValue('P6', 'Net');
...
while($row	=	$result->fetch_assoc()){
   ...
   $qtno=$row1['quet_num'];
  $u= "select * from add_qot where quet_num='$qtno'";
  $result2	=$db->query($u) or die(mysql_error());
   ...
   $ds="select * from dpr where store_code='$store_code'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, mb_strtoupper($row10['store_name'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, mb_strtoupper($store_code,'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, mb_strtoupper($row10['format_type'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, mb_strtoupper($row2['ro_no'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, mb_strtoupper($row2['ro_date'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, mb_strtoupper($row2['falt_desc'],'UTF-8'));
		$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowCount, mb_strtoupper($row['bill_date'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowCount, mb_strtoupper($row2['tot_base'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('N'.$rowCount, mb_strtoupper($row2['tot_ser'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('O'.$rowCount, mb_strtoupper($row2['tot_gst'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, mb_strtoupper($row2['net'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('Q'.$rowCount, mb_strtoupper($row1['user'],'UTF-8'));
...
header('Content-Disposition: attachment;filename="apraiseinvoice"'.$date.'".xlsx"'); //tell browser what's the file name
