...
include 'PHPExcel-1.8/Classes/PHPExcel/Writer/Excel2007.php';
 $tsname=$_GET['user'];
	...
	or ($tsname=='sumanthpotluri')){
    $y="select distinct quet_num,user from klrequest_amnt where confirm!='Yes' order by id desc";
    }else{
   $y="select distinct quet_num,user  from klrequest_amnt where confirm!='Yes' and user='$tsname' order by id desc";
	 }
...
while($row	=	$result->fetch_assoc()){
	...
	$tyy="select sum(req_amnt+gstamt) as rsamt from klrequest_amnt where quet_num='$qot' and confirm='Pending'";
