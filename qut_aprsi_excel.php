...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='apbilling')){
		 $y="select * from qot_bill where status='RUn Paid' ";
		 }else{
		 $y="select * from qot_bill where status='RUn Paid' and user='$tsname' ";
		}
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'AP Raised Invoice LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, mb_strtoupper($row['inv_date'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, mb_strtoupper($row['inv_sub_date'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, mb_strtoupper($row['speriod'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, mb_strtoupper($row['inv_sub_date'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, mb_strtoupper('AP','UTF-8'));
...
header('Content-Disposition: attachment;filename="apraisedinvoicelist.xlsx"'); //tell browser what's the file name
