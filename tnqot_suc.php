 ...
 $iname2 = $_FILES['img2']['name'];
	...
	$iname5 = ($iname5);
	 ...
	 if($qt_no1!=''){
	  ...
	  $a="INSERT INTO `add_tnqot`(`store_code`, `inv_date`, `quet_num`, `tot_base`, `tot_ser`, `tot_gst`, `net`,  `ses`,falt_no,falt_date,falt_desc,status,img1,img2,img3)
    ...
    $sqq=mysqli_query($link,"select `count` as ids from qutcount where state='TN' ");
	$rr=mysqli_fetch_array($sqq);
	$idd=$rr['ids'];
	$cc=$idd+1;
	 $qt_nox=21220000+1+$idd;
	$qt_no="QJSTSTN$qt_nox";  
	
	
	if($qt_no1!=''){
	     $ac=mysqli_query($link,"update add_tnqot set quet_num='$qt_no1' where id='$sno'");
	 }else{
	     mysqli_query($link,"update qutcount set `count`='$cc' where state='TN'");
	$ac=mysqli_query($link,"update add_tnqot set quet_num='$qt_no' where id='$sno'");
	 }
...
$model=$_POST['model'][$i];
 ...
 $query = "INSERT INTO add_tnqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap,brand,model) 
	VALUES 
...
`desc1`='".addslashes($pname)."', `hsn`='$hsn', `gst`='$gst', `uom`='$uom', `qty`='$qty', `gst_amnt`='$gst_amnt',
 ...
 $res=mysqli_query($link,$query) or die(mysqli_error($link));
  ...
  $query = "INSERT INTO add_tnqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap) 
	VALUES 
