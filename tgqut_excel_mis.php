...
$objPHPExcel = PHPExcel_IOFactory::load("misciliniustemplate.xlsx");
...
$r1=mysqli_fetch_array($ssq1);
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('B7', 'Jyothi Infra Service');
/*if(date('m') >= 03) {
...
$objPHPExcel->getActiveSheet()->setCellValue('B9','Telangana State');
$objPHPExcel->getActiveSheet()->setCellValue('B10',$format_type);
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header("Content-Disposition: attachment;filename=TS Summary sheet Miscellenious ".$quet_num." ".date('d-m-Y').".xlsx");
header('Cache-Control: max-age=0');
include_once('dbconnection/connection.php');
$aa="select  @acount:=@acount+1 sno,'$city','$store_code',falt_no,falt_date,falt_desc,quet_num,'',tot_base,tot_ser,tot_gst,(tot_base+tot_ser+tot_gst) as total  from (SELECT @acount:= 0) AS acount,add_tgqot where id='$id' ";
