...
$objPHPExcel->getActiveSheet()->mergeCells('A1:R1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KERALA WORK TO BE STARTED QUOTATIONS LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, mb_strtoupper($row['status'],'UTF-8'));
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('R'.$rowCount, mb_strtoupper($row['fault_desc'],'UTF-8'));
	$rowCount++;
$i++; }
...
header('Content-Disposition: attachment;filename="klworktobestartd.xlsx"'); //tell browser what's the file name
