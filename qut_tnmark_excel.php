...
$objPHPExcel->getActiveSheet()->mergeCells('A1:K1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'TN  QUOTATION  MARK NOT REQUIRED LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$r="select * from add_tnqot where quet_num='$q'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, mb_strtoupper($row1['superwisor'],'UTF-8'));
	 $uy="select sum(approve_amnt) as amt from tnrequest_amnt where quet_num='$q'";
...
header('Content-Disposition: attachment;filename="tnmarklist.xlsx"'); //tell browser what's the file name
