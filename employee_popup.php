			...
			function stateChanged() {
				if (xmlHttp.readyState == 4) {
					...
					if (strar.length > 0) {
						...
						opener.document.getElementById("state").value = strar[1];
...
<td align="left"><input name="submit" type="submit" value="" style="background:url(images/go1.gif);width:42px;height:22px;border-style:none;" /></td></tr>
		...
		<table width="100%" cellpadding="0" cellspacing="0" border="1" id="expense_table" style="font-size:14px;">
			...
			if (isset($_REQUEST['submit'])) {
				...
				if ($n != "") {
					$sq = mysqli_query($link, "select  empid,emp_name,state from emp where state='$state' and status != 'inactive'");
				}
				if ($sq) {
			...
			?>
						<tr height="25px">
							<td style="text-align:center;"><input type="radio" name="empid" value="<?php echo $a ?>" onclick="javascript:showDoc(this.value);" /><?php echo $emid ?></td>
							<td><?php echo $h ?></td>
							<td><?php echo $r; ?></td>
						</tr>
				...
				$x = "select empid,emp_name,state,employeeid from emp where state='$state' and status != 'inactive'";
					...
					?>
						<tr height="25px">
							<td style="text-align:center;">
								<input type="radio" name="empid" value="<?php echo $a ?>" onclick="javascript:showDoc(this.value);" /><?php echo $emid ?>
							</td>
							<td><?php echo $h ?></td>
							<td><?php echo $r; ?></td>
		...
		<?php if ($rows == 0) { ?>
			<table align="center" style="margin-left:250px;">
				<tr>
					<th style="color:#FF0000; " align="center">
