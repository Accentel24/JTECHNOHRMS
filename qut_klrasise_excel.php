...
$objPHPExcel->getActiveSheet()->mergeCells('A1:Q1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KERALA TO BE RAISE  QUOTATION LIST');
...
while($row	=	$result->fetch_assoc()){
    $q=$row['quet_num'];
   $k= "select * from klrequest_amnt where quet_num='$q'";
   $result1	=$db->query($k) or die(mysql_error());
   $row1	=	$result1->fetch_assoc();
   $qtno=$row1['quet_num'];
  $u= "select * from add_klqot where quet_num='$qtno'";
...
header('Content-Disposition: attachment;filename="klraiseinvoice"'.$date.'".xlsx"'); //tell browser what's the file name
