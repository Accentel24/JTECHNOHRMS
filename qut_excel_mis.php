...
$objPHPExcel->getActiveSheet()->setCellValue('B9','Andhra Pradesh');
$objPHPExcel->getActiveSheet()->setCellValue('B10',$format_type);
ob_clean();
ob_start(); # added 
 //header('Content-Type: application/vnd.ms-excel');
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header("Content-Disposition: attachment;filename=AP Summary sheet Miscellenious ".$quet_num." ".date('d-m-Y').".xlsx");
header('Cache-Control: max-age=0');
include_once('dbconnection/connection.php');
$aa="select  @acount:=@acount+1 sno,'$city','$store_code',falt_no,falt_date,falt_desc,quet_num,'',tot_base,tot_ser,tot_gst,(tot_base+tot_ser+tot_gst) as total  from (SELECT @acount:= 0) AS acount,add_qot where id='$id' ";
