...
if(isset($_POST['submit'])){
	...
	$tr=mysqli_fetch_array($t);
	 $empemail=$tr['emp_email'];
	  $a="INSERT INTO `add_qot`(`store_code`, `inv_date`, `quet_num`, `tot_base`, `tot_ser`, `tot_gst`, `net`,  `ses`,falt_no,falt_date,falt_desc,status)
...
$sqq=mysqli_query($link,"select max(id) as ids from add_qot ");
	...
	$ac=mysqli_query($link,"update add_qot set quet_num='$qt_no' where id='$sno'");
...
if($mark=="yes"){
    ...
    $kp=mysqli_query($link,"update add_qot set status='Not Required',markfile='$iname1',wd='$wd' where id='$id' ");
...
if($ad!=$adv_amnt and $adv_amnt>=1){
	...
	values('$qt_no','$adv_amnt','$adv_date','$ses','AP','$ac_det1','Pending','$gst_amount','$gst_type')");
}
if($ad1!=$adv_amnt1 and $adv_amnt1>=1){
	
	$ssq2=mysqli_query($link,"insert into request_amnt(quet_num,req_amnt,req_date,user,state,ac_det,confirm,gstamt,gsttype)
	values('$qt_no','$adv_amnt1','$adv_date1','$ses','AP','$ac_det2','Pending','$gst_amount1','$gst_type1')");
}
if($ad2!=$adv_amnt2 and $adv_amnt2>=1){
	
	$ssq2=mysqli_query($link,"insert into request_amnt(quet_num,req_amnt,req_date,user,state,ac_det,confirm,gstamt,gsttype)
	values('$qt_no','$adv_amnt2','$adv_date2','$ses','AP','$ac_det3','Pending','$gst_amount2','$gst_type2')");
}
