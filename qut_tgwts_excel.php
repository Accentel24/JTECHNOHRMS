...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('Q'.$rowCount, mb_strtoupper($row['ses'],'UTF-8'));
		$objPHPExcel->getActiveSheet()->SetCellValue('Q'.$rowCount, mb_strtoupper($row['falt_desc'],'UTF-8'));
		$rowCount++;
$i++; }
...
header('Content-Disposition: attachment;filename="tgwtslist.xlsx"'); //tell browser what's the file name
