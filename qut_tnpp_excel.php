...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='tnbilling')  or ($tsname=='sumanthpotluri')){
      $y="select * from tnqot_bill where status='Un Paid' ";   
    }else{
      $y="select * from tnqot_bill where status='Un Paid' and user='$tsname' ";
	 }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'TAMILNADU PAYPEMENT PENDING LIST');
...
header('Content-Disposition: attachment;filename="tnpaymentpending.xlsx"'); //tell browser what's the file name
