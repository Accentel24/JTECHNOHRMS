...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='tgbilling')){
		 $y="select * from tgqot_bill where status='RUn Paid' ";
		 }else{
		 $y="select * from tgqot_bill where status='RUn Paid' and user='$tsname' ";
		}
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'TELANGANA Raised Invoice LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, mb_strtoupper('TG','UTF-8'));
...
header('Content-Disposition: attachment;filename="tgraisedinvoicelist.xlsx"'); //tell browser what's the file name
