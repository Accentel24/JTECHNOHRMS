...
$objPHPExcel->getActiveSheet()->mergeCells('A1:R1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'TAMILNADU DOCUMENT REQUIRED LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$ac=$row['ac_det'];
    $yt="select approve_amnt,ac_det,transfer_date from tnrequest_amnt where quet_num='$qtno' and  confirm='Yes'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('O'.$rowCount, $array);
	 $a10="select sum(approve_amnt) as req_amnt from tnrequest_amnt where quet_num='$qtno' and  confirm='Yes' ";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('P'.$rowCount, mb_strtoupper($row3['req_amnt'],'UTF-8'));
	 $a11="select transfer_date from tnrequest_amnt where quet_num='$qtno' and  confirm='Yes' order by id desc limit 1 ";
	$result11=$db->query($a11) or die(mysql_error());
	 	$row4=$result11->fetch_assoc();
	$objPHPExcel->getActiveSheet()->SetCellValue('Q'.$rowCount, mb_strtoupper(date('d-m-Y',strtotime($row4['transfer_date'])),'UTF-8'));
...
header('Content-Disposition: attachment;filename="tndocument"'.$dt.'".xlsx"'); //tell browser what's the file name
